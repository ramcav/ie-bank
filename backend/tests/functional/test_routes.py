...
def test_get_account(testing_client):
    ...
    """
    _ = testing_client.post('/accounts', json={'name': 'John Doe', 'currency': '€', 'country':'Senegal'})
...
def test_create_account(testing_client):
    ...
    THEN check the response is valid
    """
    response = testing_client.post('/accounts', json={'name': 'John Doe', 'currency': '€', 'country':'Senegal'})
    assert response.status_code == 200
...
def test_create_account_data(testing_client):
    ...
    assert account.country == 'Senegal'
